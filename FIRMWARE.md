...
Only compiled by the `hw364a-selftest` environment. It hand-builds 802.11 frames in memory and runs
them through the real `sniffCb` at boot, so the detection logic can be checked on hardware without a
camera in range. See docs/TESTING.md.
