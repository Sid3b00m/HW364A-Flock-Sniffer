...
// ---------------------------------------------------------------- self-test
//
// Build the hw364a-selftest environment to feed synthetic 802.11 frames through
// the real sniffCb at boot and print PASS/FAIL over serial. This exercises the
// parsing, matching and table paths on real hardware without needing a camera
// in range. Never flash this build to a device you intend to actually use.
...
; Same firmware plus a synthetic-frame self-test that runs once at boot and
; reports over serial. Not part of default_envs, so it is never what ships.
[env:hw364a-selftest]
extends = env:hw364a
build_flags = -Wall -D SELFTEST_FRAMES
```
