# flock-mini one-shot installer

Extracts the source, installs PlatformIO, finds your board's COM port, compiles, and flashes -
in one command.
...
Short lines on purpose: long pasted lines get wrapped by the console and break.

Open PowerShell in the project folder first: Shift + right-click inside the folder in File Explorer,
...
`$s.Length` should print a number around 9000. If it prints `0`, the extraction failed and the
install will not work - tell me rather than continuing.

The installer then creates a **Install flock-mini** icon on your Desktop. Every run after this is a
double-click on that icon.

## What it does

1. Checks Python and that `FIRMWARE.md` is present.
2. Extracts `flock-mini.ino`, `flock_sigs.h`, and `platformio.ini` from `FIRMWARE.md`, backing up
   anything it would overwrite.
3. Installs PlatformIO if it is missing (roughly 300 MB of toolchain on first build).
4. Finds your CH340 port, ignoring the legacy COM1/COM2 motherboard ports, and pins it into
   `platformio.ini`.
5. Compiles.
6. Asks before flashing, then flashes.
7. Offers to open the serial monitor.

Safe to re-run. It stops at the first failure rather than plowing ahead.

## Options

- `.\install.ps1 -Port COM7` - skip auto-detection and use this port.
- `.\install.ps1 -NoFlash` - compile only, do not touch the board.
- `.\install.ps1 -KeepSource` - do not re-extract from `FIRMWARE.md`. **Use this after you edit the
  sketch by hand**, for example when you set `SELFTEST_OUI`, otherwise your edit gets overwritten.
- `.\install.ps1 -Monitor` - open the serial monitor when finished.
- `.\install.ps1 -Yes` - no prompts, for unattended runs.

Example, the self-test cycle from the install guide:

```
.\install.ps1 -KeepSource -Yes -Monitor
```
