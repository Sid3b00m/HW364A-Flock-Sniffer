# flock-mini GUI installer (Windows .exe)

A point-and-click window: pick your COM port, click **Install and Flash**, watch the log.
Same work as `install.ps1`, just visual.

Two ways to run it. Both use the same file, `flock_installer.py`.

## Option A - just run it (no exe, nothing to build)

From a PowerShell window opened in the project folder:

```powershell
python flock_installer.py
```

Tkinter ships with Python on Windows, so there is nothing to install. This is the fastest path and
avoids every downside listed below.

## Option B - build the .exe

```powershell
pip install pyinstaller
pyinstaller --onefile --windowed --name flock-mini-installer --add-data "FIRMWARE.md;." flock_installer.py
```

The result is `dist\flock-mini-installer.exe`, around 12 MB, double-clickable. `--add-data` bakes
`FIRMWARE.md` into the exe, so it is self-contained: copy it anywhere and it can still write out the
firmware source.

Know what you are getting into:

- **SmartScreen will block it.** The exe is unsigned. Click *More info* then *Run anyway*. Signing
  requires a code-signing certificate, which costs money annually.
- **Antivirus false positives are common** with PyInstaller one-file builds, because malware authors
  use the same packer. Option A has none of this.
- **The target machine still needs Python.** The exe bundles its own interpreter for the GUI, but it
  shells out to a real Python to install and run PlatformIO. Fine on your machine; not a
  "works on a bare PC" installer.

If you ever want to hand a working detector to someone with no toolchain at all, the answer is not
this exe - it is to build once, then share the compiled `firmware.bin` with a flasher like
[ESP Web Tools](https://esphome.github.io/esp-web-tools/), which flashes from a browser. Say the
word and I will set that up.
...
Same trick as the other files - this extracts `flock_installer.py` out of this document:

```
$f = [string][char]0x60 * 3
$d = if (Test-Path .\docs\GUI_INSTALLER.md) { '.\docs' } else { '.' }
$m = Get-Content "$d\GUI_INSTALLER.md" -Raw
$s = [regex]::Match($m, "(?ms)^${f}python\r?\n(.*?)^${f}").Groups[1].Value
[IO.File]::WriteAllText("$pwd\flock_installer.py", $s, (New-Object System.Text.UTF8Encoding($false)))
python flock_installer.py
```

That last line opens the GUI straight away. Build the exe only if you actually want a
